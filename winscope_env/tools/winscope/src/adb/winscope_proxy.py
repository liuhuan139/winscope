...
def get_popen_session_kwargs():
    if IS_WINDOWS:
        return {'creationflags': subprocess.CREATE_NEW_PROCESS_GROUP}
    return {'start_new_session': True}

def send_interrupt_signal(process):
    if IS_WINDOWS:
...
class FileMatcher:
    ...
    def get_filepaths(self, device_id) -> list[str]:
        ...
        files = [f.strip() for f in matchingFiles.split('\n') if f.strip()]
...
class TraceConfig:
    ...
    def execute_optional_config_command(self, server, device_id, shell, command, config_key, config_value):
        process = subprocess.Popen(shell, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                    stdin=subprocess.PIPE, **get_popen_session_kwargs())
...
def call_adb(params: str, device: str = None, stdin: bytes = None):
    ...
    try:
        ...
        return subprocess.check_output(command, stderr=subprocess.STDOUT, input=stdin).decode('utf-8').replace('\r\n', '\n').replace('\r', '')
...
class TraceThread(threading.Thread):
    def __init__(self, trace_name: str, device_id: str, command: str, trace_identifier: str, status_filename: str, trace_stop: str = ''):
        ...
        try:
            ...
            self.process = subprocess.Popen(shell, stdout=subprocess.PIPE,
                                            stderr=subprocess.PIPE, stdin=subprocess.PIPE, **get_popen_session_kwargs())
        except OSError as ex:
            raise AdbError(
    ...
    def end_trace(self):
        if self._keep_alive_timer:
            self._keep_alive_timer.cancel()
        log.info("Ending the {} process on {}".format(
            self.trace_name,
            self._device_id))
        if IS_WINDOWS:
            try:
                if self.trace_stop:
                    stop_cmd = f"shell su root sh -c '{self.trace_stop}'"
                    log.debug(f"Running stop command via separate adb: {stop_cmd}")
                    call_adb(stop_cmd, device=self._device_id)
                call_adb(f"shell su root sh -c 'echo TRACE_OK > {self.status_filename}'", device=self._device_id)
            except Exception as e:
                log.warning(f"Error running stop commands for {self.trace_name}: {e}")
            self.process.kill()
        else:
            send_interrupt_signal(self.process)
            try:
    ...
    def _read_pipe(self, pipe, result):
        try:
    ...
    def run(self):
        ...
        self.reset_timer()
        self.process.stdin.write(self.trace_command)
        self.process.stdin.flush()
        out_buf, err_buf = [], []
        out_reader = threading.Thread(target=self._read_pipe, args=(self.process.stdout, out_buf))
        err_reader = threading.Thread(target=self._read_pipe, args=(self.process.stderr, err_buf))
        out_reader.start()
        err_reader.start()
        self.process.wait()
        out_reader.join(timeout=5)
        err_reader.join(timeout=5)
        self.out = out_buf[0] if out_buf else b''
        self.err = err_buf[0] if err_buf else b''
    ...
    def process_with_device(self, server, path, device_id):
        ...
        for t, config in trace_targets:
            ...
            for trace_identifier in trace_identifiers:
                ...
                thread = TraceThread(t.trace_name, device_id, command.encode('utf-8'), trace_identifier, t.status_filename, t.trace_stop)
