...
export class TracePipeline {
  ...
  private async loadUnzippedArchive(
    ...
    console.error('[WINSCOPE-DEBUG] loadUnzippedArchive called with', unzippedArchive.length, 'files');
    unzippedArchive.forEach((f) => {
      console.error('[WINSCOPE-DEBUG] File:', f.file.name, 'size:', f.file.size);
    });

    const filterResult = await this.traceFileFilter.filter(unzippedArchive);
    console.error('[WINSCOPE-DEBUG] Filter result - perfetto:', filterResult.perfetto?.file.name, 'legacy count:', filterResult.legacy.length);
    filterResult.legacy.forEach((f) => {
      console.error('[WINSCOPE-DEBUG] Legacy file:', f.file.name, 'size:', f.file.size);
    });
    ...
    if (!filterResult.perfetto && filterResult.legacy.length === 0) {
      console.error('[WINSCOPE-DEBUG] No valid files found!');
      UserNotifier.add(new NoValidFiles());
      return;
    }
    ...
    console.error('[WINSCOPE-DEBUG] Legacy parsers created:', legacyParsers.length);

    let perfettoParsers: FileAndParsers | undefined;

    if (filterResult.perfetto) {
      console.error('[WINSCOPE-DEBUG] Attempting to create perfetto parsers for:', filterResult.perfetto.file.name);
      try {
        ...
        console.error('[WINSCOPE-DEBUG] Perfetto parsers created:', parsers.length);
        perfettoParsers = new FileAndParsers(filterResult.perfetto, parsers);
      } catch (e) {
        console.error('[WINSCOPE-DEBUG] Failed to create perfetto parsers:', e);
      }
    }
